...
import cz.janrossler.sorts.sortable.RadixSort;
import cz.janrossler.sorts.sortable.Sortable;

public class RadixAsync extends AsyncTask<String, String, String> {
    private Context context;
    public SortingService.Callbacks callbacks;

    public RadixAsync(Context context){
        this.context = context;
    }

    protected String doInBackground(@NonNull String... sessions) {
        RadixSort sort = new RadixSort(context, sessions[0]);
        sort.setSortingListener(new Sortable.SortingListener() {
            ...
            public void onSuccessSort(int seconds) {
                ...
                try {
                    numberManager.saveSortingToSession(sessions[0], Sortable.RADIX, seconds, array);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                publishProgress();
