...
import cz.janrossler.sorts.sortable.CountingSort;
...
public class CountingAsync extends AsyncTask<String, String, String> {
    ...
    protected String doInBackground(@NonNull String... sessions) {
        CountingSort sort = new CountingSort(context, sessions[0]);
        sort.setSortingListener(new Sortable.SortingListener() {
            ...
            public void onSuccessSort(int seconds) {
                ...
                try {
                    numberManager.saveSortingToSession(sessions[0], Sortable.COUNTING, seconds, array);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                isDone = true;
