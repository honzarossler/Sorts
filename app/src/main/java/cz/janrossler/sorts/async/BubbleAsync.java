...
import cz.janrossler.sorts.sortable.BubbleSort;
import cz.janrossler.sorts.sortable.Sortable;

public class BubbleAsync extends AsyncTask<String, String, String> {
    private Context context;
    public static ProgressDialog pDialog;
    public SortingService.Callbacks callbacks;
    private boolean isDone = false;

    public BubbleAsync(Context context){
    ...
    protected String doInBackground(@NonNull String... sessions) {
        BubbleSort sort = new BubbleSort(context, sessions[0]);
        sort.setSortingListener(new Sortable.SortingListener() {
            ...
            public void onSuccessSort(int seconds) {
                ...
                try {
                    numberManager.saveSortingToSession(sessions[0], Sortable.BUBBLE, seconds, array);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                isDone = true;
